...
public class Time extends TimeBase<Time> {
	...
	public Date toDate() {
		...
		c.setTimeInMillis((long) (this.toSec() * (double) SECS_TO_MILLI));
		return c.getTime();
	}
	...
	public static Time fromDate(Date date) {
		return Time.fromSec(((double) date.getTime()) * MILLI_TO_SECS);
	}
	...
	public static Time fromJsonObject(JsonObject jsonObject) {
		// check the fields
		int secs = jsonObject.containsKey(FIELD_SECS) ? jsonObject
				.getInt(FIELD_SECS) : 0;
		int nsecs = jsonObject.containsKey(FIELD_NSECS) ? jsonObject
				.getInt(FIELD_NSECS) : 0;
